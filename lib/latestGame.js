...
function* latestLOL (summonerName, region, params) {
  ...
  const neutralMinions = stats.neutralMinionsKilled || 0
  const wardsPlaced = stats.wardPlaced || 0
  const wardsKilled = stats.wardKilled || 0

  const score = `${kills} / ${deaths} / ${assists}`
  const kda = ((kills + assists) / deaths).toFixed(2)
  const cs = stats.minionsKilled + neutralMinions
  const wards = `${wardsPlaced} / ${wardsKilled}`
